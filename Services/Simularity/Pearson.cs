...
{
    public class Pearson : Simularity
    {
        public Pearson(List<Tuple<double, double>> values) : base(values)
        {
        }
        ...
        public override double Calculate()
        {
            ...
            var denominator = Math.Sqrt(sumX2 - Math.Pow(sumX, 2) / length) * Math.Sqrt(sumY2 - Math.Pow(sumY, 2) / length);
